...
Like many, I was fascinated by the 2025 Oscars race. In particular, one video^[https://www.youtube.com/watch?v=3Ep7ftG4MfQ] sparked my interest for this project. It claimed Ariana Grande was unlikely to win the *Academy Award for Best Supporting Actress* since she had not won any of the four awards preceding the Oscars, which are usually good indicators for the final winners. Curious, I wanted to investigate this by looking at previous award seasons. That is, I wanted to know: What can the data tell us?
...
oscar_prob_plot <- ggplot(award_summary, aes(x = Result, 
                                             y = Proportion, 
                                             fill = Result)) +
  geom_col() +
...
# Save graph
ggsave("oscar_prob_plot.png",
       plot = oscar_prob_plot,
...
# Plot award combinations
combo_plot <- ggplot(combo, aes(x = combo, 
                                y = n)) +
  geom_col() +
...
Our logistic regression model is given by $\log(\frac{P(Y = 1 \mid \vec{X})}{P(Y = 0 \mid \vec{X})}) = \beta_0 + \beta_1 X_1 + \ldots + \beta_4 X_4$. Interestingly, $\hat{\beta}_4 = 4.6514755$ implies a SAG win multiplies the odds of winning the Oscar by $e^{4.6514755} = 104.739$, holding all the other awards constant.
...
# Save decision tree
png("dec_tree_plot.png", width = 8, height = 6, units = "in", res = 300, bg = "white")

# Plot decision tree
dec_tree_plot <- fancyRpartPlot(best_supp_actress_tree, 
                                cex = 0.8, 
                                caption = "", 
                                type = 2)
```
...
best_supp_actress_long_sag_loss <- best_supp_actress_sag_loss %>%
  mutate(across(c(BAFTA, Critics_Choice, Golden_Globe),
                ~ ifelse(. == 1, "Win", "Loss"))) %>%
...
# Plot probability of Oscar win, conditioning on each non-Oscar result
oscar_prob_sag_loss_plot <- ggplot(award_summary_sag_loss, aes(x = Result,
                                                               y = Proportion,
                                                               fill = Result)) +
  geom_col() +
  geom_text(aes(label = scales::percent(Proportion, accuracy = 1)),
...
# Save graph
ggsave("oscar_prob_sag_loss_plot.png",
       plot = oscar_prob_sag_loss_plot,
       width = 8, height = 6, dpi = 300,
       bg = "white")
```
